...
async function fetchServerInfo() {
    try {
        const host = (window && window.WS_CONFIG && window.WS_CONFIG.host) ? window.WS_CONFIG.host : window.location.hostname;
        const httpPort = (window && window.WS_CONFIG && window.WS_CONFIG.httpPort) ? window.WS_CONFIG.httpPort : 8080;
        ...
        // Fallback: show a sensible connect string using WS_CONFIG or defaults
        try {
            const host = (window && window.WS_CONFIG && window.WS_CONFIG.host) ? window.WS_CONFIG.host : 'localhost';
            const mcPort = (window && window.WS_CONFIG && window.WS_CONFIG.mcPort) ? window.WS_CONFIG.mcPort : (window && window.WS_CONFIG && window.WS_CONFIG.minecraftPort) ? window.WS_CONFIG.minecraftPort : 19131;
            const connectionDiv = document.querySelector('#info div[style*="font-size:12px"]');
            if (connectionDiv) {
                connectionDiv.innerHTML = `Left-drag: Rotate | Scroll: Zoom<br>Connect: <strong>/connect ${host}:${mcPort}</strong>`;
            }
