...
export function displayAnalysisResults(data) {
    ...
    // Ensure modal/backdrop are visible so results are not rendered into an offscreen/hidden container
    try {
        if (resultsDiv) { resultsDiv.style.display = 'block'; resultsDiv.classList.add('show'); }
        if (backdrop) backdrop.classList.add('show');
    } catch (e) { console.error('Failed to show assessment modal elements', e); }
    ...
    try {
        for (const [player, analysis] of Object.entries(data.analyses)) {
            let parsedContent = '';
            try {
                parsedContent = parseMarkdownToHTML(analysis || '');
            } catch (parseErr) {
                console.error('Markdown parsing failed for player', player, parseErr);
                parsedContent = `<div style="color: #f44336;">(Failed to parse analysis content for ${player})</div>`;
            }
        ...
        console.error('Failed while building analysis HTML', e);
        if (contentDiv) contentDiv.innerHTML = `<div style="color: #f44336;">Error rendering analysis results: ${e.message || e}</div>`;
        return;
    }
