    ...
    ws.onmessage = (event) => {
        ...
        switch (data.type) {
            ...
            case 'analysis_result':
                try {
                    // Log a small preview of the analysis payload to aid debugging in DevTools
                    try {
                        const preview = (data && data.analyses) ? Object.keys(data.analyses).slice(0,5) : (typeof data === 'string' ? data.substring(0,200) : JSON.stringify(data).substring(0,200));
                        console.log('WS analysis_result received - preview:', preview);
                    } catch (previewErr) {
                        console.log('WS analysis_result received');
                    }
                    if (events && typeof events.triggerAnalysisResult === 'function') events.triggerAnalysisResult(data);
            ...
            case 'active_players':
                try {
                    console.log('WS active_players received, count:', Array.isArray(data.players) ? data.players.length : 0);
                    const list = Array.isArray(data.players) ? data.players : [];
                    const authoritative = new Set();

                    for (const p of list) {
                        try {
                            const pid = (p && (p.playerId || p.id || p.player_id));
                            if (!pid) continue;
                            const pidStr = String(pid);
                            authoritative.add(pidStr);
                            const pname = p.playerName || p.player_name || p.name || pidStr;

                            // If the renderer doesn't know about this player create it, otherwise update name/position
                            if (players && typeof players.createPlayer === 'function' && (!players.players || !players.players.has(pidStr))) {
                                players.createPlayer(pidStr, pname);
                            }

                            if (players && typeof players.updatePlayer === 'function') {
                                if (typeof p.x === 'number' && typeof p.y === 'number' && typeof p.z === 'number') {
                                    // server sends Minecraft coords -> updatePlayer will convert
                                    players.updatePlayer(pidStr, pname, p.x, p.y, p.z, false);
                                } else {
                                    // ensure name is synchronized
                                    try { if (players.players && players.players.has(pidStr)) players.players.get(pidStr).name = pname; } catch (e) {}
                                }
                            }
                        } catch (perr) { console.error('processing active_players entry failed', perr, p); }
                    }

                    // Remove any local players not present in authoritative list
                    try {
                        if (players && players.players) {
                            for (const [existingId] of players.players) {
                                if (!authoritative.has(existingId)) {
                                    if (players && typeof players.removePlayer === 'function') players.removePlayer(existingId);
                                }
                            }
                        }
                    } catch (cleanupErr) { console.error('active_players cleanup failed', cleanupErr); }

                    // Update UI
                    try { if (players && typeof players.updatePlayerCount === 'function') players.updatePlayerCount(); if (players && typeof players.updatePlayerList === 'function') players.updatePlayerList(); } catch (uiErr) { console.error('active_players UI update failed', uiErr); }
                } catch (e) {
                    console.error('active_players handling failed', e);
                }
                break;
